const assetSearchTextCache=new WeakMap();
function assetSearchText(asset){
  if(assetSearchTextCache.has(asset))return assetSearchTextCache.get(asset);
  const value=JSON.stringify(asset).toLowerCase();assetSearchTextCache.set(asset,value);return value;
}
function renderAssets(){
  const q=(document.getElementById('assets-filter').value||'').trim().toLowerCase(),life=document.getElementById('assets-lifecycle-filter')?.value||'all';
  const securityByIdentity=new Map(),securityByIP=new Map();
  for(const x of Array.isArray(assetSecurity)?assetSecurity:[]){
    if(x.asset_identity)securityByIdentity.set(`${x.sensor_id}\x00${x.asset_identity}`,x);
    if(x.asset_ip)securityByIP.set(`${x.sensor_id}\x00${x.asset_ip}`,x);
  }
  const data=assets.filter(a=>(!q||assetSearchText(a).includes(q))&&(life==='all'||(life==='recent'&&a.IdentityFresh!==false)||(life==='quiet'&&a.IdentityFresh===false)));
  document.getElementById('assets-count').textContent=data.length+' assets';
  document.querySelector('#table-assets tbody').innerHTML=data.map(a=>{
    const identity=a.CanonicalIdentity||((a.MAC||'').trim()?`mac:${String(a.MAC).toLowerCase()}`:`ip:${a.IP||''}`),security=securityByIdentity.get(`${a.SensorID}\x00${identity}`)||securityByIP.get(`${a.SensorID}\x00${a.IP||''}`);
    const securityHTML=security?`<span class="pill severity-${security.status==='infected'?'high':security.status==='suspected'?'medium':'low'}">${esc(String(security.status||'unknown').toUpperCase())}</span>`:assetSecurityLoaded?'clean/unknown':'<span class="status-pill">loading…</span>';
    return `<tr class="asset-row ${a.Confirmed===false?'row-unconfirmed':''}" data-sensor="${esc(a.SensorID)}" data-mac="${esc(a.MAC)}" data-vendor="${esc(a.Vendor||'')}" data-ip="${esc(a.IP||'')}"><td><input class="asset-check" type="checkbox" data-sensor="${esc(a.SensorID)}" data-mac="${esc(a.MAC)}" ${selected.has(a.SensorID+'::'+a.MAC)?'checked':''}></td><td>${esc(a.SensorID)}</td><td>${esc(a.IP)}</td><td>${esc(a.MAC)}</td><td>${esc(a.Vendor)}</td><td>${esc(a.Hostname)}</td><td><span class="status-pill ${a.IdentityFresh===false?'degraded':'healthy'}">${a.IdentityFresh===false?'quiet':'recent'}</span></td><td class="${a.Confirmed===false?'state-new':'state-ok'}">${a.Confirmed===false?'NEW / UNCONFIRMED':'confirmed'}</td><td>${a.IsOT?'OT':'IT'}</td><td>${esc((a.Protocols||[]).join(', '))}</td><td>${esc(a.VLANID||'untagged')}</td><td>${esc(a.Score??1)}</td><td>${(a.IsHoneypot===true||Number(a.Score??1)>=Number(a.HoneypotThreshold??100))?'<span class="pill honeypot">HONEYPOT</span>':Number(a.Score??1)>=75?'<span class="pill severity-high">CRITICAL</span>':Number(a.Score??1)>=40?'<span class="pill severity-medium">ELEVATED</span>':'standard'}</td><td>${esc(a.PacketCount)}</td><td>${time(a.LastSeen)}</td><td>${securityHTML}</td><td>${can('asset_confirm_delete')?`<button class="danger-btn infect-one" data-sensor="${esc(a.SensorID)}" data-ip="${esc(a.IP)}">Tag infection</button> `:''}${a.Confirmed===false&&can('asset_confirm_delete')?`<button class="ack-btn confirm-one" data-sensor="${esc(a.SensorID)}" data-mac="${esc(a.MAC)}">Confirm</button>`:a.Confirmed===false?'pending':'—'}</td></tr>`;
  }).join('');
  updateBulk();
}
...
document.getElementById('devices-import-file').addEventListener('change',async e=>{
  ...
  try{
    ...
    alert(`Imported ${r.applied} row(s)${Number(r.skipped||0)>0?` · skipped ${r.skipped} invalid row(s)`:``}.`);
    refreshAll();
...
function alertIdentity(a){return String(a?.ID??a?.AlertKey??'')}
const selectedAlerts=new Set();
let alertTableRows=[];
const alertSearchState={q:'',sensor_id:'',status:'',activity:'',severity:'',from:'',to:'',sort:'newest',limit:100,offset:0,total:0};
function syncAlertSensorFilter(){}
function alertSearchQuery(){const p=new URLSearchParams();if(alertSearchState.q)p.set('q',alertSearchState.q);if(alertSearchState.sensor_id)p.set('sensor_id',alertSearchState.sensor_id);if(alertSearchState.status)p.set('status',alertSearchState.status);if(alertSearchState.activity)p.set('activity',alertSearchState.activity);if(alertSearchState.severity)p.set('severity',alertSearchState.severity);if(alertSearchState.from)p.set('from',alertSearchState.from);if(alertSearchState.to)p.set('to',alertSearchState.to);p.set('sort',alertSearchState.sort);p.set('limit',String(alertSearchState.limit));p.set('offset',String(alertSearchState.offset));return p.toString()}
async function refreshAlertSearch(){syncAlertSensorFilter();const result=await api('/alerts/search?'+alertSearchQuery());alertSearchState.total=Number(result?.total)||0;alertSearchState.limit=Number(result?.limit)||alertSearchState.limit;alertSearchState.offset=Number(result?.offset)||0;if(alertSearchState.total>0&&alertSearchState.offset>=alertSearchState.total){alertSearchState.offset=Math.floor((alertSearchState.total-1)/alertSearchState.limit)*alertSearchState.limit;return refreshAlertSearch()}if(alertSearchState.total===0)alertSearchState.offset=0;alertTableRows=Array.isArray(result?.items)?result.items:[];renderAlerts()}
function readAlertFilters(){alertSearchState.q=document.getElementById('alerts-filter-q').value.trim();alertSearchState.sensor_id=document.getElementById('alerts-filter-sensor').value;alertSearchState.status=document.getElementById('alerts-filter-status').value;alertSearchState.activity=document.getElementById('alerts-filter-activity').value;alertSearchState.severity=document.getElementById('alerts-filter-severity').value;alertSearchState.from=document.getElementById('alerts-filter-from').value;alertSearchState.to=document.getElementById('alerts-filter-to').value;alertSearchState.sort=document.getElementById('alerts-filter-sort').value||'newest'}
function updateAlertBulkBar(){const count=selectedAlerts.size;document.getElementById('alerts-approve').hidden=!count;document.getElementById('alerts-confirm').hidden=!count;document.getElementById('alerts-selection-count').textContent=count?`${count} selected`:'';const selectable=alertTableRows.filter(a=>(a.Status||'new')==='new');const all=document.getElementById('alerts-all');all.checked=selectable.length>0&&selectable.every(a=>selectedAlerts.has(`${a.SensorID}::${alertIdentity(a)}`));all.indeterminate=selectable.some(a=>selectedAlerts.has(`${a.SensorID}::${alertIdentity(a)}`))&&!all.checked}
function renderAlertBadge(){const badge=document.getElementById('alert-badge');if(!badge)return;const unreviewed=Number.isFinite(Number(alertStats?.unreviewed))?Number(alertStats.unreviewed):0;badge.textContent=unreviewed?String(unreviewed):'';badge.title=unreviewed?`${unreviewed.toLocaleString()} unreviewed alert${unreviewed===1?'':'s'}`:'No unreviewed alerts';badge.setAttribute('aria-label',badge.title)}
function renderAlerts(){document.querySelector('#table-alerts tbody').innerHTML=alertTableRows.map((a,i)=>{const key=`${a.SensorID}::${alertIdentity(a)}`,status=(a.Status||'new'),isNew=status==='new',active=!!a.Active;return `<tr class="alert-row alert-status-${esc(status)}" data-index="${i}"><td>${isNew?`<input type="checkbox" class="alert-select" data-key="${esc(key)}" ${selectedAlerts.has(key)?'checked':''} aria-label="Select alert ${esc(alertIdentity(a))}">`:'—'}</td><td>${esc(a.SensorID)}</td><td><span class="severity ${esc(a.Severity)}">${esc(a.Severity)}</span></td><td>${esc(a.Type)}</td><td>${esc(a.Message)}</td><td>${esc(a.IP)}</td><td>${esc(a.Count)}</td><td><span class="health-badge ${active?'healthy':'offline'}">${active?'ACTIVE':'RESOLVED'}</span></td><td>${esc(status)}</td><td>${time(a.LastSeen)}</td></tr>`}).join('');renderAlertBadge();const total=alertSearchState.total,start=total?alertSearchState.offset+1:0,end=Math.min(total,alertSearchState.offset+alertTableRows.length),overall=Number(alertStats?.total)||total,unreviewed=Number(alertStats?.unreviewed)||0,active=Number(alertStats?.active)||0;const summary=document.getElementById('alerts-table-summary');if(summary)summary.textContent=`${total.toLocaleString()} matching · ${active.toLocaleString()} active (last 5m) · ${unreviewed.toLocaleString()} unreviewed · ${overall.toLocaleString()} retained`;document.getElementById('alerts-page-info').textContent=total?`${start.toLocaleString()}–${end.toLocaleString()} of ${total.toLocaleString()}`:'0 records';const pages=Math.max(1,Math.ceil(total/alertSearchState.limit)),page=Math.floor(alertSearchState.offset/alertSearchState.limit)+1;document.getElementById('alerts-page-number').textContent=`Page ${page} of ${pages}`;document.getElementById('alerts-page-prev').disabled=alertSearchState.offset<=0;document.getElementById('alerts-page-next').disabled=alertSearchState.offset+alertSearchState.limit>=total;document.getElementById('alerts-page-size').value=String(alertSearchState.limit);updateAlertBulkBar()}
...
function openAlertDetail(index){const a=alertTableRows[index];if(!a)return;document.getElementById('alert-detail-title').textContent=`Alert · ${a.Type||'unknown'}`;document.getElementById('alert-detail-body').innerHTML=`<dl class="status-list detail-status-list"><div><dt>Sensor</dt><dd>${esc(a.SensorID)}</dd></div><div><dt>Alert key</dt><dd class="wrap-anywhere">${esc(a.AlertKey||a.ID||'—')}</dd></div><div><dt>Severity</dt><dd><span class="severity ${esc(a.Severity)}">${esc(a.Severity)}</span></dd></div><div><dt>Activity</dt><dd>${a.Active?'Active':'Resolved / inactive'}</dd></div><div><dt>Review status</dt><dd>${esc(a.Status||'new')}</dd></div><div><dt>IP</dt><dd>${esc(a.IP||'—')}</dd></div><div><dt>Episodes</dt><dd>${esc(a.Count||1)}</dd></div><div><dt>First seen</dt><dd>${time(a.FirstSeen)}</dd></div><div><dt>Last seen</dt><dd>${time(a.LastSeen)}</dd></div>${a.ApprovedBy?`<div><dt>Reviewed by</dt><dd>${esc(a.ApprovedBy)}</dd></div>`:''}</dl><h3>Message</h3><div class="detail-message">${esc(a.Message||'—')}</div>${a.Evidence&&Object.keys(a.Evidence).length?`<h3>Structured evidence</h3><dl class="status-list detail-status-list">${objectRows(a.Evidence)}</dl>`:''}`;document.getElementById('alert-detail-modal').hidden=false}

async function openAlertsWithFilters({severity='',activity='',q='',sensor='' }={}){
  document.getElementById('alerts-filter-q').value=q||'';
  document.getElementById('alerts-filter-sensor').value=sensor||'';
  document.getElementById('alerts-filter-status').value='';
  document.getElementById('alerts-filter-activity').value=activity||'';
  document.getElementById('alerts-filter-severity').value=severity||'';
  document.getElementById('alerts-filter-from').value='';
  document.getElementById('alerts-filter-to').value='';
  document.getElementById('alerts-filter-sort').value='newest';
  readAlertFilters();alertSearchState.offset=0;selectedAlerts.clear();
  await refreshAlertSearch();
}
async function openAlertByKey(sensor,id){
  const wanted=String(id||'');if(!wanted)return false;
  await openAlertsWithFilters({q:wanted,sensor:sensor||''});
  const index=alertTableRows.findIndex(a=>alertIdentity(a)===wanted&&(!sensor||String(a.SensorID)===String(sensor)));
  if(index<0)return false;
  openAlertDetail(index);return true;
}
...
document.getElementById('alerts-filter-apply').onclick=async()=>{readAlertFilters();alertSearchState.offset=0;selectedAlerts.clear();await refreshAlertSearch()};
document.getElementById('alerts-filter-clear').onclick=async()=>{for(const id of ['alerts-filter-q','alerts-filter-from','alerts-filter-to'])document.getElementById(id).value='';for(const id of ['alerts-filter-sensor','alerts-filter-status','alerts-filter-activity','alerts-filter-severity'])document.getElementById(id).value='';document.getElementById('alerts-filter-sort').value='newest';readAlertFilters();alertSearchState.offset=0;selectedAlerts.clear();await refreshAlertSearch()};
document.getElementById('alerts-filter-q').addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();document.getElementById('alerts-filter-apply').click()}});
document.getElementById('alerts-page-size').onchange=async e=>{alertSearchState.limit=Number(e.target.value)||100;alertSearchState.offset=0;selectedAlerts.clear();await refreshAlertSearch()};
document.getElementById('alerts-page-prev').onclick=async()=>{alertSearchState.offset=Math.max(0,alertSearchState.offset-alertSearchState.limit);await refreshAlertSearch()};
document.getElementById('alerts-page-next').onclick=async()=>{if(alertSearchState.offset+alertSearchState.limit<alertSearchState.total){alertSearchState.offset+=alertSearchState.limit;await refreshAlertSearch()}};
