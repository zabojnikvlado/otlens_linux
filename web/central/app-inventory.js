function renderAssets(){const q=document.getElementById('assets-filter').value.toLowerCase(),life=document.getElementById('assets-lifecycle-filter')?.value||'all',data=assets.filter(a=>JSON.stringify(a).toLowerCase().includes(q)&&(life==='all'||(life==='recent'&&a.IdentityFresh!==false)||(life==='quiet'&&a.IdentityFresh===false)));document.getElementById('assets-count').textContent=data.length+' assets';document.querySelector('#table-assets tbody').innerHTML=data.map(a=>`<tr class="asset-row ${a.Confirmed===false?'row-unconfirmed':''}" data-sensor="${esc(a.SensorID)}" data-mac="${esc(a.MAC)}" data-vendor="${esc(a.Vendor||'')}" data-ip="${esc(a.IP||'')}"><td><input class="asset-check" type="checkbox" data-sensor="${esc(a.SensorID)}" data-mac="${esc(a.MAC)}" ${selected.has(a.SensorID+'::'+a.MAC)?'checked':''}></td><td>${esc(a.SensorID)}</td><td>${esc(a.IP)}</td><td>${esc(a.MAC)}</td><td>${esc(a.Vendor)}</td><td>${esc(a.Hostname)}</td><td><span class="status-pill ${a.IdentityFresh===false?'degraded':'healthy'}">${a.IdentityFresh===false?'quiet':'recent'}</span></td><td class="${a.Confirmed===false?'state-new':'state-ok'}">${a.Confirmed===false?'NEW / UNCONFIRMED':'confirmed'}</td><td>${a.IsOT?'OT':'IT'}</td><td>${esc((a.Protocols||[]).join(', '))}</td><td>${esc(a.VLANID||'untagged')}</td><td>${esc(a.Score??1)}</td><td>${(a.IsHoneypot===true||Number(a.Score??1)>=Number(a.HoneypotThreshold??100))?'<span class="pill honeypot">HONEYPOT</span>':Number(a.Score??1)>=75?'<span class="pill severity-high">CRITICAL</span>':Number(a.Score??1)>=40?'<span class="pill severity-medium">ELEVATED</span>':'standard'}</td><td>${esc(a.PacketCount)}</td><td>${time(a.LastSeen)}</td><td>${(()=>{const x=assetSecurity.find(v=>v.sensor_id===a.SensorID&&v.asset_ip===a.IP);return x?`<span class="pill severity-${x.status==='infected'?'high':x.status==='suspected'?'medium':'low'}">${esc(x.status.toUpperCase())}</span>`:'clean/unknown'})()}</td><td>${can('asset_confirm_delete')?`<button class="danger-btn infect-one" data-sensor="${esc(a.SensorID)}" data-ip="${esc(a.IP)}">Tag infection</button> `:''}${a.Confirmed===false&&can('asset_confirm_delete')?`<button class="ack-btn confirm-one" data-sensor="${esc(a.SensorID)}" data-mac="${esc(a.MAC)}">Confirm</button>`:a.Confirmed===false?'pending':'—'}</td></tr>`).join('');updateBulk()}
