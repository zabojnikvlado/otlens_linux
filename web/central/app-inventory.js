...
let alertTableRows=[];
const alertSearchState={q:'',sensor_id:'',status:'',severity:'',from:'',to:'',sort:'newest',limit:100,offset:0,total:0};
function syncAlertSensorFilter(){}
function alertSearchQuery(){const p=new URLSearchParams();if(alertSearchState.q)p.set('q',alertSearchState.q);if(alertSearchState.sensor_id)p.set('sensor_id',alertSearchState.sensor_id);if(alertSearchState.status)p.set('status',alertSearchState.status);if(alertSearchState.severity)p.set('severity',alertSearchState.severity);if(alertSearchState.from)p.set('from',alertSearchState.from);if(alertSearchState.to)p.set('to',alertSearchState.to);p.set('sort',alertSearchState.sort);p.set('limit',String(alertSearchState.limit));p.set('offset',String(alertSearchState.offset));return p.toString()}
