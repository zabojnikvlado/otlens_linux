...
function renderRules(){
  ...
  tbody.innerHTML=rules.map(r=>{
    ...
    return `<tr class="clickable-row rule-detail-row" data-sensor="${esc(r.SensorID)}" data-rule="${esc(r.ID)}" title="Open rule details">
      ...
      <td class="rule-hit-count">${Number(r.HitCount||0).toLocaleString()}</td>
  ...
  }).join('')||'<tr><td colspan="8" class="empty-state">No detection rules available.</td></tr>';
}
...
function ruleDetailHTML(rule){
  ...
  <div class="rule-detail-layout">
    ...
    <section class="rule-detail-section"><h3>Runtime</h3><dl class="rule-detail-dl"><dt>Hits</dt><dd>${Number(rule.HitCount||0).toLocaleString()}</dd><dt>Last hit</dt><dd>${time(rule.LastHit)}</dd><dt>Simulation hits</dt><dd>${Number(rule.SimulationHits||0).toLocaleString()}</dd><dt>Last simulation</dt><dd>${time(rule.LastSimulationHit)}</dd></dl></section>
