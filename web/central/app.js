...
async function queueRecon(sensor,targets,profile='safe-discovery',options={}){
 ...
 const policy=standalone?{allowed_networks:[],denied_targets:[],ports:[22,80,443,445,3389,502,102,44818,4840],packets_per_second:5,concurrent_targets:1,timeout_seconds:3,require_manual_approval:false,ot_protocols:[],credential_id:'',authenticated_methods:[]}:{allowed_networks:split(document.getElementById('recon-networks')?.value),denied_targets:split(document.getElementById('recon-denied')?.value),ports:split(document.getElementById('recon-ports')?.value||'22,80,443,445,3389,502,102,44818,4840').map(Number).filter(x=>x>0&&x<65536),packets_per_second:Number(document.getElementById('recon-rate')?.value||5),concurrent_targets:1,timeout_seconds:Number(document.getElementById('recon-timeout')?.value||3),require_manual_approval:Boolean(document.getElementById('recon-manual-approval')?.checked),ot_protocols:profile==='ot-conservative'?ot:[],credential_id:profile==='authenticated-inventory'?(document.getElementById('recon-credential')?.value||''):'',authenticated_methods:profile==='authenticated-inventory'&&document.getElementById('recon-auth-ssh')?.checked?['ssh']:[]};
...
function renderAssetPanel(panel){if(!activeAssetDetail)return;const a=activeAssetDetail,r=assetRecon(a),body=document.getElementById('asset-detail-body');document.querySelectorAll('.asset-detail-tabs button').forEach(x=>x.classList.toggle('active',x.dataset.assetPanel===panel));if(panel==='identity')body.innerHTML=`<div class="identity-grid">${[['Hostname',r.hostname],['Vendor',r.vendor],['Model',r.model],['Operating system',r.os],['Firmware',r.firmware],['Serial',r.serial],['Classification',a.IsOT?'OT':'IT'],['Last profiled',r.last?time(r.last):'Never']].map(x=>`<div><span>${x[0]}</span><strong>${esc(x[1]||'Unknown')}</strong></div>`).join('')}</div><h3>OT identity</h3><dl class="status-list">${Object.entries(r.ot||{}).map(([k,v])=>`<div><dt>${esc(k)}</dt><dd>${esc(v)}</dd></div>`).join('')||'<div><dt>Status</dt><dd>No OT identity evidence</dd></div>'}</dl>`;else if(panel==='services')body.innerHTML=`<div class="scrollable"><table class="data-table"><thead><tr><th>Port</th><th>Transport</th><th>Service</th><th>Product</th><th>Banner / TLS</th></tr></thead><tbody>${(r.services||[]).map(x=>`<tr><td>${x.port}</td><td>${esc(x.transport)}</td><td>${esc(x.service)}</td><td>${esc(x.product||x.version||'—')}</td><td>${esc(x.banner||x.tls_subject||'—')}</td></tr>`).join('')||'<tr><td colspan="5">No active service evidence</td></tr>'}</tbody></table></div>`;else if(panel==='evidence')body.innerHTML=`<div class="evidence-list">${(r.evidence||[]).map(x=>`<article><div><strong>${esc(x.field)}</strong><span>${esc(x.value)}</span></div><div><span>${esc(x.source)}</span><span class="confidence">${Number(x.confidence||0)}%</span><time>${time(x.observed_at)}</time></div></article>`).join('')||'<div class="empty-dashboard">No reconnaissance evidence</div>'}</div>`;else {const jobs=(reconnaissanceJobs||[]).filter(j=>j.sensor_id===a.SensorID&&(j.targets||[]).includes(a.IP));body.innerHTML=`<div class="scrollable"><table class="data-table"><thead><tr><th>Time</th><th>Profile</th><th>Status</th><th>Hostname</th><th>Vendor / model</th><th>OS / firmware</th></tr></thead><tbody>${jobs.map(j=>{const x=(j.results||[]).find(v=>v.target===a.IP)||{};return `<tr><td>${time(j.created_at)}</td><td>${esc(j.profile)}</td><td>${esc(j.status)}</td><td>${esc(x.hostname||'—')}</td><td>${esc([x.vendor,x.model].filter(Boolean).join(' / ')||'—')}</td><td>${esc([x.os,x.firmware].filter(Boolean).join(' / ')||'—')}</td></tr>`}).join('')||'<tr><td colspan="6">No active profiling history</td></tr>'}</tbody></table></div>`}}
