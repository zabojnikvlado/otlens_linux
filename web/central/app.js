...
function renderSMB(){const q=(document.getElementById('smb-filter')?.value||'').toLowerCase(),mode=document.getElementById('smb-risk')?.value||'';const rows=smbObservations.filter(x=>(!q||JSON.stringify(x).toLowerCase().includes(q))&&(!mode||(mode==='risk'&&smbRisk(x))||(mode==='encrypted'&&(x.IsEncrypted||x.is_encrypted))));document.getElementById('smb-count').textContent=`${rows.length} observations`;window.__smbRows=rows;document.querySelector('#table-smb tbody').innerHTML=rows.map((x,i)=>{const flags=[(x.IsAdminShare||x.is_admin_share)?'ADMIN SHARE':'',(x.IsExecutable||x.is_executable)?'EXECUTABLE':'',(x.IsScript||x.is_script)?'SCRIPT':'',(x.IsEncrypted||x.is_encrypted)?'ENCRYPTED':'',(x.StreamGapped||x.stream_gapped)?'STREAM GAP':'',(x.StreamResynced||x.stream_resynced)?'RESYNC':''].filter(Boolean);return `<tr class="smb-row ${smbRisk(x)?'row-threat':''}" data-index="${i}"><td>${time(x.Timestamp||x.timestamp)}</td><td>${esc(x.sensor_id||x.SensorID)}</td><td>${esc(x.ClientIP||x.client_ip)} → ${esc(x.ServerIP||x.server_ip)}</td><td>${esc(x.Command||x.command)}</td><td>${esc(x.ShareName||x.share_name||'—')}</td><td>${esc(x.FileName||x.file_name||x.NamedPipe||x.named_pipe||'—')}</td><td>${esc(x.Bytes||x.bytes||0)}</td><td>${esc(x.Status||x.status||'—')}</td><td>${flags.map(f=>`<span class="pill severity-${f==='ENCRYPTED'?'medium':'high'}">${esc(f)}</span>`).join(' ')||'—'}</td></tr>`}).join('')}
function openSMBDetail(x){if(!x)return;const v=(...names)=>names.map(n=>x[n]).find(z=>z!==undefined&&z!==null&&z!=='')??'—';document.getElementById('smb-detail-title').textContent=`SMB ${v('Command','command')} · ${v('ClientIP','client_ip')} → ${v('ServerIP','server_ip')}`;const fields=[['Observed',time(v('Timestamp','timestamp'))],['Sensor',v('sensor_id','SensorID')],['Client',v('ClientIP','client_ip')],['Server',v('ServerIP','server_ip')],['Command',v('Command','command')],['Request command',v('RequestCommand','request_command')],['Request matched',v('RequestMatched','request_matched')],['Share',v('ShareName','share_name')],['File',v('FileName','file_name')],['Named pipe',v('NamedPipe','named_pipe')],['Session ID',v('SessionID','session_id')],['Tree ID',v('TreeID','tree_id')],['Message ID',v('MessageID','message_id')],['Persistent File ID',v('FileIDPersistent','file_id_persistent')],['Volatile File ID',v('FileIDVolatile','file_id_volatile')],['Dialect',v('Dialect','dialect')],['Status',v('Status','status')],['Bytes',v('Bytes','bytes')],['Encrypted',v('IsEncrypted','is_encrypted')],['Signed',v('IsSigned','is_signed')],['Stream gap',v('StreamGapped','stream_gapped')],['Parser resynchronized',v('StreamResynced','stream_resynced')]];document.getElementById('smb-detail-body').innerHTML=`<div class="smb-detail-grid">${fields.map(([k,val])=>`<article><small>${esc(k)}</small><strong class="wrap-anywhere">${esc(val)}</strong></article>`).join('')}</div><p class="rules-help">Request/response correlation, TreeID/FileID context and stream integrity fields are shown when available from the SMB stream parser.</p>`;document.getElementById('smb-detail-modal').hidden=false}
...
function ruleScope(r){return r.Scope||r.scope||'Universal'}
function ruleCategoryLabel(v){return ({network_behavior:'Network behavior',it_activity:'IT activity',ot_process:'OT process activity',asset_intelligence:'Asset intelligence',architecture_violation:'Architecture violation',custom:'Custom',network:'Network',ics:'ICS',asset:'Asset',ot_tag:'OT tag',security:'Security',baseline:'Baseline'})[v]||v||'—'}
function filteredRules(){const scope=document.getElementById('rule-filter-scope')?.value||'all',status=document.getElementById('rule-filter-status')?.value||'all',severity=document.getElementById('rule-filter-severity')?.value||'all',category=document.getElementById('rule-filter-category')?.value||'all',q=(document.getElementById('rule-filter-search')?.value||'').trim().toLowerCase();return (rules||[]).filter(r=>{const rs=ruleScope(r),st=r.Simulation?'simulation':(r.Enabled?'enabled':'disabled'),text=[r.Name,r.Description,r.Category,...(r.Protocols||[]),...(r.AttackMapping||[])].join(' ').toLowerCase();return (scope==='all'||rs===scope)&&(status==='all'||st===status)&&(severity==='all'||String(r.Severity||'').toLowerCase()===severity)&&(category==='all'||r.Category===category)&&(!q||text.includes(q))})}
function renderRules(){const all=rules||[];document.getElementById('rule-kpi-all').textContent=all.length;document.getElementById('rule-kpi-it').textContent=all.filter(r=>ruleScope(r)==='IT').length;document.getElementById('rule-kpi-ot').textContent=all.filter(r=>ruleScope(r)==='OT').length;document.getElementById('rule-kpi-itot').textContent=all.filter(r=>ruleScope(r)==='IT/OT').length;document.getElementById('rule-kpi-universal').textContent=all.filter(r=>ruleScope(r)==='Universal').length;document.getElementById('rule-kpi-enabled').textContent=all.filter(r=>r.Enabled&&!r.Simulation).length;document.querySelector('#table-rules tbody').innerHTML=filteredRules().map(r=>{const custom=String(r.Kind).toLowerCase()==='custom',mode=r.Simulation?`simulation (${r.SimulationHits||0} matches)`:(r.Enabled?'enabled':'disabled'),toggleLabel=r.Enabled?'Disable rule':'Enable rule',scope=ruleScope(r),scopeClass=scope==='IT/OT'?'itot':scope.toLowerCase();return `<tr title="${esc((r.AttackMapping||[]).join(', '))}"><td>${esc(r.SensorID)}</td><td><strong>${esc(r.Name)}</strong>${r.Description?`<br><small>${esc(r.Description)}</small>`:''}<br><span class="rule-origin rule-origin-${custom?'custom':'builtin'}">${custom?'custom':'built-in'}</span></td><td><span class="rule-scope-badge rule-scope-${scopeClass}">${esc(scope)}</span></td><td>${esc(ruleCategoryLabel(r.Category||r.Kind))}</td><td class="rule-protocols">${esc((r.Protocols||[]).join(', ')||'—')}</td><td class="rule-condition rule-condition-summary">${esc(ruleSummary(r))}</td><td>${esc(mode)}</td><td>${esc(r.Severity||'—')}</td><td>${esc(r.HitCount||0)}</td><td>${time(r.LastHit)}</td><td class="rule-actions"><button type="button" class="rule-state-toggle ${r.Enabled?'is-on':'is-off'}" data-sensor="${esc(r.SensorID)}" data-id="${esc(r.ID)}" data-enabled="${r.Enabled?'true':'false'}" aria-pressed="${r.Enabled?'true':'false'}" aria-label="${toggleLabel}" title="${toggleLabel}" ${can('rule_manage')?'':'disabled'}><span aria-hidden="true"></span></button>${custom&&can('rule_manage')?`<button class="secondary-btn rule-edit" data-sensor="${esc(r.SensorID)}" data-id="${esc(r.ID)}">Edit</button><button class="danger-btn rule-delete" data-sensor="${esc(r.SensorID)}" data-id="${esc(r.ID)}">Delete</button>`:custom?'<span class="builtin-lock">view only</span>':'<span class="builtin-lock">built-in</span>'}</td></tr>`}).join('')||'<tr><td colspan="11">No rules match the selected filters.</td></tr>'}
...
function resetRuleForm(){document.getElementById('rule-form').reset();document.getElementById('rule-id').value='';document.getElementById('rule-priority').value='100';document.getElementById('rule-scope').value='Universal';document.getElementById('rule-protocols').value='';document.getElementById('rule-attack-mapping').value='';document.getElementById('rule-enabled').checked=true;document.getElementById('rule-groups').innerHTML='';addGroup();document.getElementById('rule-form-error').textContent='';document.getElementById('rule-test-result').textContent='';document.getElementById('rule-modal-title').textContent='Create detection rule'}
function openRuleModal(rule=null){populateRuleSensors();resetRuleForm();if(rule){document.getElementById('rule-modal-title').textContent='Edit detection rule';document.getElementById('rule-id').value=rule.ID;document.getElementById('rule-sensor').value=rule.SensorID;document.getElementById('rule-sensor').disabled=true;document.getElementById('rule-name').value=rule.Name||'';document.getElementById('rule-description').value=rule.Description||'';document.getElementById('rule-scope').value=ruleScope(rule);document.getElementById('rule-category').value=rule.Category||'custom';document.getElementById('rule-protocols').value=(rule.Protocols||[]).join(', ');document.getElementById('rule-attack-mapping').value=(rule.AttackMapping||[]).join(', ');document.getElementById('rule-severity').value=rule.Severity||'medium';document.getElementById('rule-priority').value=rule.Priority||100;document.getElementById('rule-enabled').checked=!!rule.Enabled;document.getElementById('rule-simulation').checked=!!rule.Simulation;document.getElementById('rule-group-operator').value=rule.GroupOperator||'AND';document.getElementById('rule-suppression').value=(rule.Suppression&&rule.Suppression.Mode)||'aggregate';document.getElementById('rule-interval').value=(rule.Suppression&&rule.Suppression.IntervalSeconds)||600;document.getElementById('rule-groups').innerHTML='';ruleGroupsOf(rule).forEach(g=>addGroup({operator:g.Operator||g.operator,conditions:g.Conditions||g.conditions}))}else document.getElementById('rule-sensor').disabled=false;document.getElementById('rule-modal').hidden=false;toggleRuleInterval()}
...
['rule-filter-scope','rule-filter-status','rule-filter-severity','rule-filter-category'].forEach(id=>document.getElementById(id).onchange=renderRules);document.getElementById('rule-filter-search').oninput=renderRules;document.querySelector('.rule-kpis').onclick=e=>{const b=e.target.closest('button');if(!b)return;if(b.dataset.ruleScope){document.getElementById('rule-filter-scope').value=b.dataset.ruleScope;renderRules()}if(b.dataset.ruleStatus){document.getElementById('rule-filter-status').value=b.dataset.ruleStatus;renderRules()}};
...
function renderSettings(){
  const extra=document.getElementById('settings-runtime-config');
  if(!extra)return;
  const groups=settings.RuntimeConfig||{};
  extra.innerHTML=Object.entries(groups).map(([group,values])=>`<section class="settings-config-group"><h3>${esc(group)}</h3><dl class="status-list">${Object.entries(values||{}).map(([k,v])=>`<div><dt>${esc(k)}</dt><dd class="wrap-anywhere">${esc(Array.isArray(v)?v.join(', '):typeof v==='object'?JSON.stringify(v):v)}</dd></div>`).join('')}</dl></section>`).join('')||'<div class="empty-dashboard">No runtime configuration is available.</div>';
}
...
async function refreshAll(){
  // Keep the last stable state while polling; only switch to offline after a failed refresh.
  ...
  if(!rejected.length){setConn(true,'live');document.getElementById('conn-text').title=''}
    ...
    document.getElementById('conn-text').title=allUnauthorized?'Your session has expired — please log in again':allForbidden?'The current role cannot access the requested API endpoints':allNetwork?'Central backend is unreachable':'Failed endpoints: '+rejected.map(x=>x.path).join(', ');
    if(allUnauthorized){
...
async function queueRecon(sensor,targets,profile='safe-discovery'){
 const split=v=>String(v||'').split(/[\s,;]+/).map(x=>x.trim()).filter(Boolean), ot=[...document.querySelectorAll('#recon-ot-protocols input:checked')].map(x=>x.value);
 return api('/reconnaissance/jobs',{method:'POST',body:JSON.stringify({sensor_id:sensor,targets,profile,policy:{allowed_networks:split(document.getElementById('recon-networks')?.value),denied_targets:split(document.getElementById('recon-denied')?.value),ports:split(document.getElementById('recon-ports')?.value||'22,80,443,445,3389,502,102,44818,4840').map(Number).filter(x=>x>0&&x<65536),packets_per_second:Number(document.getElementById('recon-rate')?.value||5),concurrent_targets:1,timeout_seconds:Number(document.getElementById('recon-timeout')?.value||3),require_manual_approval:Boolean(document.getElementById('recon-manual-approval')?.checked),ot_protocols:profile==='ot-conservative'?ot:[],credential_id:profile==='authenticated-inventory'?(document.getElementById('recon-credential')?.value||''):'',authenticated_methods:profile==='authenticated-inventory'&&document.getElementById('recon-auth-ssh')?.checked?['ssh']:[]}})});
}
...
function openReconJob(id){const j=(reconnaissanceJobs||[]).find(x=>x.id===id);if(!j)return;document.getElementById('recon-job-title').textContent=`${j.id} · ${j.profile}`;const results=j.results||[];document.getElementById('recon-job-body').innerHTML=`<div class="identity-grid">${[['Sensor',j.sensor_id],['Status',j.status],['Created',time(j.created_at)],['Started',j.started_at?time(j.started_at):'—'],['Completed',j.completed_at?time(j.completed_at):'—'],['Targets',(j.targets||[]).length],['Rate',`${j.policy?.packets_per_second||'—'} probes/s`],['Timeout',`${j.policy?.timeout_seconds||'—'} s`]].map(x=>`<div><span>${x[0]}</span><strong>${esc(x[1])}</strong></div>`).join('')}</div><h3>Results</h3><div class="scrollable"><table class="data-table"><thead><tr><th>Target</th><th>Reachable</th><th>Hostname</th><th>Vendor / model</th><th>OS / firmware</th><th>OT identity</th><th>Error</th></tr></thead><tbody>${results.map(x=>`<tr><td>${esc(x.target)}</td><td>${x.reachable?'yes':'no'}</td><td>${esc(x.hostname||'—')}</td><td>${esc([x.vendor,x.model].filter(Boolean).join(' / ')||'—')}</td><td>${esc([x.os,x.firmware].filter(Boolean).join(' / ')||'—')}</td><td>${esc(Object.entries(x.ot_identity||{}).map(([k,v])=>`${k}: ${v}`).join(', ')||'—')}</td><td>${esc(x.error||'—')}</td></tr>`).join('')||'<tr><td colspan="7">No result uploaded yet</td></tr>'}</tbody></table></div>`;document.getElementById('recon-job-modal').hidden=false}
...
async function renderAssetPanel(panel){
  ...
  }else if(panel==='vulnerabilities'){
    body.innerHTML='<div class="empty-dashboard">Loading vulnerability matches…</div>';
    if(!r.vendor){body.innerHTML='<div class="empty-dashboard">No vendor is identified for this asset, so vendor-based vulnerability matching is not available.</div>';return}
    try{
      const result=await api('/assets/vulnerabilities?vendor='+encodeURIComponent(r.vendor));
      if(!result.Loaded){body.innerHTML='<div class="empty-dashboard">No vulnerability snapshot is loaded on Central. Configure or import a vulnerability source on the Vulnerabilities tab.</div>';return}
      const list=Array.isArray(result.Advisories)?result.Advisories:[];
      body.innerHTML=list.length?list.map(v=>`<div class="modal-history"><b>${esc(v.CVEID)}</b> <span class="severity ${esc(String(v.Severity||'').toLowerCase())}">${esc(v.Severity||'—')}</span><br>${esc(v.Title)}<br><small>${esc(v.Product||'—')} · ${esc(v.PublishedDate||'—')}</small>${v.URL?` · <a href="${esc(v.URL)}" target="_blank" rel="noopener">advisory</a>`:''}</div>`).join(''):'<div class="empty-dashboard">No advisories match the currently identified vendor.</div>';
    }catch(err){body.innerHTML=`<div class="empty-dashboard">Failed to load vulnerability matches: ${esc(err.message)}</div>`}
  }else if(panel==='ip-history'){
    body.innerHTML='<div class="empty-dashboard">Loading IP history…</div>';
    if(!a.MAC){body.innerHTML='<div class="empty-dashboard">This asset has no MAC address, so stable IP history cannot be queried.</div>';return}
    try{
      const hist=await api(`/sensors/${encodeURIComponent(a.SensorID)}/assets/${encodeURIComponent(a.MAC)}/ip-history`),rows=Array.isArray(hist)?hist:[];
      body.innerHTML=rows.length?`<table class="data-table"><thead><tr><th>IP</th><th>First seen</th><th>Last seen</th></tr></thead><tbody>${rows.map(h=>`<tr><td>${esc(h.IP)}</td><td>${time(h.FirstSeen)}</td><td>${time(h.LastSeen)}</td></tr>`).join('')}</tbody></table>`:'<div class="empty-dashboard">No recorded IP history for this asset.</div>';
...
document.querySelector('#table-assets tbody')?.addEventListener('click',e=>{if(e.target.closest('input,button'))return;const row=e.target.closest('.asset-row');if(!row)return;const a=(assets||[]).find(x=>x.SensorID===row.dataset.sensor&&x.IP===row.dataset.ip);if(a)openAssetDetail(a)});
document.getElementById('asset-detail-close').onclick=()=>document.getElementById('asset-detail-modal').hidden=true;document.querySelector('.asset-detail-tabs').onclick=e=>{const b=e.target.closest('[data-asset-panel]');if(b)renderAssetPanel(b.dataset.assetPanel)};document.getElementById('asset-run-discovery').onclick=async()=>{if(!activeAssetDetail)return;try{await queueRecon(activeAssetDetail.SensorID,[activeAssetDetail.IP]);document.getElementById('asset-detail-modal').hidden=true;openDashboardTab('reconnaissance');await loadRecon()}catch(e){alert(e.parsed?.error||e.message)}};
...
document.querySelector('#table-smb tbody')?.addEventListener('click',e=>{const row=e.target.closest('.smb-row');if(row)openSMBDetail((window.__smbRows||[])[Number(row.dataset.index)])});
document.getElementById('smb-detail-close')?.addEventListener('click',()=>document.getElementById('smb-detail-modal').hidden=true);
document.getElementById('smb-detail-modal')?.addEventListener('click',e=>{if(e.target.id==='smb-detail-modal')e.target.hidden=true});
const vulnSourceForm=document.getElementById('vuln-source-form');
if(vulnSourceForm)vulnSourceForm.addEventListener('submit',async e=>{e.preventDefault();const status=document.getElementById('vuln-source-status'),url=document.getElementById('vuln-source-url').value.trim();if(!url){status.textContent='Enter a URL';return}status.textContent='Refreshing…';try{const r=await api('/vulnerabilities/source/url',{method:'POST',body:JSON.stringify({url})});status.textContent=`Loaded ${r.loaded} advisories`;await refreshAll()}catch(err){status.textContent='Refresh failed: '+err.message}});
document.getElementById('vuln-source-file')?.addEventListener('change',async e=>{const file=e.target.files[0];if(!file)return;const status=document.getElementById('vuln-source-status'),fd=new FormData();fd.append('file',file,file.name);status.textContent='Importing…';try{const r=await api('/vulnerabilities/source/upload',{method:'POST',body:fd});status.textContent=`Loaded ${r.loaded} advisories`;await refreshAll()}catch(err){status.textContent='Import failed: '+err.message}finally{e.target.value=''}});
