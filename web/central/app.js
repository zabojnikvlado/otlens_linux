...
function scheduleLiveRefresh(type){
  livePendingTypes.add(type);
  if(liveRefreshTimer)return;
  liveRefreshTimer=setTimeout(async()=>{liveRefreshTimer=null;livePendingTypes.clear();try{await refreshAll()}catch(e){console.error('live refresh',e)}},450);
}
...
async function refreshAll(){
  ...
  if(ok('/incidents'))incidents=list('/incidents');if(ok('/correlation-rules'))correlationRules=list('/correlation-rules');if(ok('/asset-risk'))assetRiskData=list('/asset-risk');if(ok('/dns-observations?limit=1000'))dnsObservations=list('/dns-observations?limit=1000');if(ok('/smb-observations?limit=1000'))smbObservations=list('/smb-observations?limit=1000');
  ...
  try{if(ok('/correlation-rules'))renderCorrelationRules()}catch(e){console.error('render correlation rules',e)}try{if(ok('/incidents'))renderIncidents()}catch(e){console.error('render incidents',e)}try{renderThreatIntel()}catch(e){console.error('render threat intel',e)}try{renderDNS()}catch(e){console.error('render dns',e)}try{renderSMB()}catch(e){console.error('render smb',e)}
  ...
  if(!rejected.length){setAPIConnection(true)}
  else{
...
document.querySelector('#table-rules tbody')?.addEventListener('click',e=>{if(e.target.closest('button,input,a'))return;const row=e.target.closest('.rule-detail-row');if(!row)return;const rule=rules.find(x=>String(x.ID)===String(row.dataset.rule)&&String(x.SensorID)===String(row.dataset.sensor));if(rule){document.getElementById('rule-detail-body').innerHTML=detailGrid(rule);document.getElementById('rule-detail-modal').hidden=false}});
...
function openRuleSetModal(){const modal=document.getElementById('ruleset-modal');modal.hidden=false;populateSensorSelect(document.getElementById('ruleset-assign-sensors'),true);const list=document.getElementById('ruleset-rule-list');list.innerHTML=rules.map((r,i)=>`<label><input type="checkbox" value="${i}" checked> ${esc(r.Name)} <small>${esc(r.SensorID)} · ${esc(r.Severity||'')}</small></label>`).join('')||'<p>No rules available.</p>'}
