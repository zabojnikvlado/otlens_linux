...
function renderTopology(){
  ...
  const mappedEdges=rawEdges.map(e=>{
    ...
    const udpRows=(udpConversations||[]).filter(x=>{const k=x.Key||{};return x.SensorID===e.SensorID&&((k.EndpointAIP===e.SrcIP&&k.EndpointBIP===e.DstIP)||(k.EndpointAIP===e.DstIP&&k.EndpointBIP===e.SrcIP))}),udpProtocols={},udpRTTs=[];
    udpRows.forEach(x=>{const p=String(x.Protocol||'udp').toUpperCase();udpProtocols[p]=(udpProtocols[p]||0)+1;if(Number(x.rtt_millis)>0)udpRTTs.push(Number(x.rtt_millis))});
    const udpNote=udpRows.length?`\n\nUDP\n${Object.entries(udpProtocols).map(([p,n])=>`${p}: ${n}`).join('\n')}${udpRTTs.length?`\nAverage RTT: ${(udpRTTs.reduce((a,b)=>a+b,0)/udpRTTs.length).toFixed(2)} ms`:''}`:'';
    return{id:e.ID,from:ip.get(e.SensorID+'::'+e.SrcIP),to:ip.get(e.SensorID+'::'+e.DstIP),_srcIP:e.SrcIP,_dstIP:e.DstIP,_sensorID:e.SensorID,label,title:(lateral?`Potential lateral movement: honeypot ${e.SrcIP} initiated communication to ${e.DstIP}`:interVlan?'Inter-VLAN communication':e.Protocol)+flowNote+udpNote,font:{color:lateral?'#ff9f95':interVlan?'#fbbf24':'#d7e1ec',strokeWidth:2,strokeColor:'#0b1220',size:dense?10:14},color:{color:lateral?'#ef4444':interVlan?'#f59e0b':e.IsOT?'#5fd1c4':'#94a3b8',opacity:dense&&!lateral&&!interVlan?.55:1},dashes:lateral?false:interVlan?[10,6]:false,width:lateral?5:interVlan?3:e.IsOT?2:1,arrows:lateral?'to':undefined,smooth:false}
  });
