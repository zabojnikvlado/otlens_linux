...
async function refreshDomains(domains,force=false){
  ...
  if(ok('/udp-conversations?active=true'))udpConversations=list('/udp-conversations?active=true');if(ok('/udp-telemetry'))udpTelemetry=results['/udp-telemetry'].value||{totals:{},protocols:{},top_protocol:''};
...
const TAB_PERMISSION_ALIAS={health:'sensors',threatintel:'alerts',dns:'alerts',smb:'alerts'};
