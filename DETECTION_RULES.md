...
| `segmentation.go` | `segmentation_violation` (high) — dve VLAN, ktorých nakonfigurované Purdue Model úrovne sú viac ako `detect.segmentation.maxleveljump` od seba, komunikujú priamo. Vypnuté defaultne, vyžaduje `detect.segmentation.vlanlevels` | `core.EventPacketParsed` | src+dst IP |
